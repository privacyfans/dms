    ...
    public function registerEvents(): array {
        ...
        return [
            AfterSheet::class => function(AfterSheet $event) {
                ...
                $sheet->setCellValue('U1', "∑ Waktu Tunggu UB - DCRM");
                $sheet->setCellValue('U4', "u");
                $sheet->mergeCells('V1:V3');
                $sheet->setCellValue('V1', "∑ Waktu Pengerjaan UB - DCRM");
                $sheet->setCellValue('V4', "v");
                $sheet->mergeCells('W1:W3');
                $sheet->setCellValue('W1', "∑ Waktu Keseluruhan");
                $sheet->setCellValue('W4', "w");
                $sheet->mergeCells('X1:AA1');
                $sheet->setCellValue('X1', "Rata-rata Estimasi Perhitungan SLA Harian");
                $sheet->mergeCells('X2:X3');
                $sheet->setCellValue('X2', "Rata-rata Waktu per Batch Time UB");
                $sheet->setCellValue('X4', "x = r / q");
                $sheet->mergeCells('Y2:Y3');
                $sheet->setCellValue('Y2', "Rata-rata Waktu per Pemenuhan Kekurangan Dokumen UB");
                $sheet->setCellValue('Y4', "y = t / s");
                $sheet->mergeCells('Z2:Z3');
                $sheet->setCellValue('Z2', "Rata-rata Waktu Tunggu DCRM per Pengajuan");
                $sheet->setCellValue('Z4', "z = u / q");
                $sheet->mergeCells('AA2:AA3');
                $sheet->setCellValue('AA2', "Rata-rata Waktu Pengerjaan DCRM per Pengajuan");
                $sheet->setCellValue('AA4', "aa = v / q");
                $sheet->mergeCells('AB1:AB3');
                $sheet->setCellValue('AB1', "Rata-rata Waktu Pengerjaan per Loan ID per Hari (Estimasi SLA harian)");
                $sheet->setCellValue('AB4', "ab = x + y + z + aa");
                $sheet->mergeCells('AC1:AC3');
                $sheet->setCellValue('AC1', "Rata-rata Waktu Pengerjaan per Loan ID per Hari (Estimasi SLA harian tanpa memperhitungkan pemenuhan kekurangan dokumen / kondisi ideal)");
                $sheet->setCellValue('AC4', "ac = z + aa");
                $sheet->mergeCells('AD1:AD3');
                $sheet->setCellValue('AD1', "Rata-rata Waktu Pengerjaan per Loan ID per Hari (Estimasi SLA Dept. DCRM (UB + DCRM))");
                $sheet->setCellValue('AD4', "ad = aa");
                $sheet->mergeCells('AE1:AE3');
                $sheet->setCellValue('AE1', "Waktu Terlama  Pengajuan Dokumen dalam 1 hari");
                $sheet->setCellValue('AE4', "ae");
                $sheet->mergeCells('AF1:AF3');
                $sheet->setCellValue('AF1', "Waktu Tercepat  Pengajuan Dokumen dalam 1 hari");
                $sheet->setCellValue('AF4', "af");
