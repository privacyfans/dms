...
@section('breadcrumb')
					...
					<div class="left-content">
						<h4 class="content-title mb-1">Setting Cut Off HQ</h4>
						<nav aria-label="breadcrumb">
							<ol class="breadcrumb">
								<li class="breadcrumb-item"><a href="#">Setting Cut Off HQ</a></li>
        ...
        <tr>
            ...
            <td>
                {{-- <form action="{{ route('branch.destroy',$br->branch_code ) }}" method="POST"> --}}
                    ...
                    {{-- <a class="btn btn-info" href="{{ route('cut_off_hq.show',$br->id) }}">Show</a> --}}
					@if(Session("role")=="spv3" || Session("role")=="spv4")
                    <form action="{{ route('cut_off_hq.update',$br->id) }}" method="POST">
                    @csrf
                    @method('PUT')
                    <div class="row">
                        <div class="col">
                    ...
                    {{-- <a class="btn btn-primary" href="{{ route('cut_off_hq.edit', $br->id) }}">Change</a> --}}
					@endif
					@if(Session("role")=="spv5")
                    {{--<a class="btn btn-secondary" href="{{ route('cut_off_hq.approve', $br->id) }}">Approve</a>--}}
                    <form action="{{ route('cut_off_hq.approveSubmit') }}" method="POST">
                    @csrf
                    @method('POST')
                    <div class="row">
                        <div class="col">
                            
                            <input type="hidden" name="id" value="{{ $br->id }}" class="form-control" placeholder="Cut Off">
