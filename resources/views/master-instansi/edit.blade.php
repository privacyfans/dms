...
<form action="{{ route('master-instansi.update', $masterInstansi->id) }}" method="POST">
    @csrf
    @method('PUT')
                    ...
                    <div class="pd-30 pd-sm-40 bg-gray-100">
                        <div class="row row-xs align-items-center mg-b-20">
                            ...
                            <div class="col-md-12 mg-t-5">
                                <input type="text" name="instansi" class="form-control" placeholder="Nama Instansi" value="{{ $masterInstansi->instansi }}">
                            </div>
                        </div>
                        <div class="row row-xs align-items-center mg-b-20">
                            <div class="col-md-12">
                            ...
                            <div class="col-md-12 mg-t-5">
                                <select class="livesearch-klasifikasi form-control p-3" name="klasifikasi_debitur">
                                    @foreach($klasifikasiList as $klas)
                                    <option value="{{ $klas->klasifikasi_debitur }}" selected="selected">{{ $klas->klasifikasi_debitur }}</option>
                                    @endforeach
                                </select>
                            </div>
                        </div>
                        ...
                        <div class="row row-xs align-items-center mg-b-20">
                            ...
                            <div class="col-md-12 mg-t-5">
                                <select class="form-control select2-no-search" name="status">
                                    <option value="active" {{ $masterInstansi->status == 'active' ? 'selected' : '' }}>Active</option>
                                    <option value="inactive" {{ $masterInstansi->status == 'inactive' ? 'selected' : '' }}>Inactive</option>
                                </select>
                            </div>
                        </div>
