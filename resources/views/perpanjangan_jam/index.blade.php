...
@section('content')
    <div class="row">
        <div class="col-lg-12 margin-tb">
            <div class="pull-left">
                <h2>List Request Perpanjangan Jam Layanan</h2>
            </div>
            <?php //@if((Session('role')=='spv1' && Session('branch_type')==1) || (Session('role')=='pcp' && Session('branch_type')==2) )?>
            @if(Session('role')=='pc' || Session('role')=='pcp')
            <div class="pull-right">
                <a class="btn btn-success" href="{{ route('perpanjangan_jam.create') }}"> Request Perpanjangan Jam Layanan</a>
            </div>
            @endif
        </div>
    </div>
...
@endif
    ...
    <table class="table table-bordered table-hover  mb-0 text-md-nowrap border">
        ...
        @foreach ($perpanjangan_jam as $br)
        <tr>
            <td>{{ ++$i }}</td>
            <td>{{ $br->branch_input." - ".$br->branchlist->branch_name }}</td>
            <td>{{ $br->jam_layanan }}</td>
            <td>{{ $br->user_input }}</td>
            <td>{{ $br->date_input }}</td>
            <td>
                @if($br->final_flag == 0)
            ...
            <td>
                <form action="{{ route('perpanjangan_jam.destroy',$br->id ) }}" method="POST">
   
                    <a class="btn btn-info" href="{{ route('perpanjangan_jam.show',$br->id) }}">Show</a>
